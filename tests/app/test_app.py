...
def test_refactored_call_pattern_report(mock_get_call_pattern_records, client,
                             interviewer_call_pattern_report):
    mock_get_call_pattern_records.return_value = interviewer_call_pattern_report
...
@patch("app.app.get_call_pattern_report_refactor")
def test_refactored_call_pattern_report_returns_error(mock_get_call_pattern_records, client):
    mock_get_call_pattern_records.side_effect = BertException("Invalid date range parameters provided", 400)
    response = client.get("/api/reports/call-pattern-refactored/matpal?start-date=2021-01-01&end-date=2021-01-01")
    assert response.status_code == 400
    assert response.get_data(as_text=True) == '{"error": "Invalid date range parameters provided"}'


@patch("app.app.get_call_pattern_report_refactor")
def test_refactored_call_pattern_report_returns_empty_dict(mock_get_call_pattern_records, client):
    mock_get_call_pattern_records.return_value = {}
    response = client.get("/api/reports/call-pattern-refactored/matpal?start-date=2021-01-01&end-date=2021-01-01")
    assert response.status_code == 200
    assert json.loads(response.get_data(as_text=True)) == {}
