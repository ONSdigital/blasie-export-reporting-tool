...
def test_get_questionnaire_name_from_id_executes_as_expected(query, config):
    ...
    # act
    QuestionnaireConfigurationTable.get_questionnaire_name_from_id(
        config, "guid_id_example_1"
    )
...
def test_get_questionnaire_name_from_id_returns_questionnaire_name_when_found(
    ...
    # act
    result = QuestionnaireConfigurationTable.get_questionnaire_name_from_id(
        config, "guid_id_example_1"
    )
...
def test_get_questionnaire_name_from_id_throws_row_not_found_exception(query, config):
    ...
    with pytest.raises(
        RowNotFound,
        match="Could not find configuration with InstrumentId: guid_id_example_1",
    ):
        QuestionnaireConfigurationTable.get_questionnaire_name_from_id(
            config, "guid_id_example_1"
        )
