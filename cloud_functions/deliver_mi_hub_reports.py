...
def deliver_mi_hub_reports_cloud_function_processor(request: flask.Request, config: Config) -> str:
    request_json = request.get_json()
    if request_json is None:
        logging.error("deliver_mi_hub_reports_cloud_function_processor was not triggered due to an invalid request")
        raise Exception("deliver_mi_hub_reports_cloud_function_processor was not triggered due to an invalid request")

    google_storage = init_google_storage(config)
    if google_storage.bucket is None:
        logging.error(f"Connection to storage bucket {config.nifi_staging_bucket} failed")
        raise Exception(f"Connection to storage bucket {config.nifi_staging_bucket} failed", 500)
